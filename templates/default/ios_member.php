...
echo '
<div class="post-list">
    <div class="item-avatar"><a href="/user/',$m_obj['id'],'"><img src="/avatar/mini/',$m_obj['avatar'],'.png" alt="',$m_obj['name'],'" /></a></div>
    <div class="item-content count',$article['comments'],'">
        <h1><a href="/topics/',$article['id'],'">',$article['title'],'</a></h1>
        <span class="item-date"><a href="/nodes/',$article['cid'],'">',$article['cname'],'</a>';
if($article['comments']){
    echo ' • <a href="/user/',$article['ruid'],'">',$article['rauthor'],'</a> ',$article['edittime'],'回复';
}else{
    echo ' • <a href="/user/',$m_obj['id'],'">',$m_obj['name'],'</a> ',$article['addtime'],'发表';
}
echo '        </span>
    </div>';
if($article['comments']){
